...
run_backup() {
  local label="$1"
  local archive="$2"
  shift 2               # remaining args passed directly to tar
  local t0=$SECONDS
  local TAR_EXIT=0
  local src_mb
  src_mb=$(du -sm "$App_Data/$label" 2>/dev/null | awk '{print $1}') || src_mb="?"
  log "--- $label (~${src_mb} MB uncompressed) => $archive"
  tar -czf "$archive" "$@" 2>/tmp/tar_warn_$$.txt || TAR_EXIT=$?
  if [ -s "/tmp/tar_warn_$$.txt" ]; then
    log "  tar warnings (normal for live databases — archive is still usable):"
  ...
  arch_mb=$(du -sm "$archive" 2>/dev/null | awk '{print $1}') || arch_mb="?"
  elapsed=$(( SECONDS - t0 ))
...
PLEX_SRV="$App_Data/plex/library/Library/Application Support/Plex Media Server"

run_backup "plex"    "$BACKUP_DIR/plex.tar.gz" \
  "$App_Data/plex" \
  --exclude="$PLEX_SRV/Drivers" \
  --exclude="$PLEX_SRV/Scanners/Credits Detection" \
  --exclude="$PLEX_SRV/Media"

run_backup "radarr"  "$BACKUP_DIR/radarr.tar.gz"  "$App_Data/radarr"
run_backup "sonarr"  "$BACKUP_DIR/sonarr.tar.gz"  "$App_Data/sonarr"
run_backup "sabnzbd" "$BACKUP_DIR/sabnzbd.tar.gz" "$App_Data/sabnzbd"
...
run_backup "plex-autodata" "$BACKUP_AUTODATA" \
  -C "$App_Data" \
  "plex/library/Library/Application Support/Plex Media Server/Drivers" \
  "plex/library/Library/Application Support/Plex Media Server/Scanners/Credits Detection" \
  "plex/library/Library/Application Support/Plex Media Server/Media"
