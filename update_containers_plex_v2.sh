...
log "Backup 1/2 — Critical data (configs, databases) => $BACKUP_ARCHIVE"
log "  Excluding: Drivers, ML models, Media index files (auto-regenerated by Plex)"
tar -czf "$BACKUP_ARCHIVE" "$App_Data" \
  --exclude="*/Plex Media Server/Drivers" \
  --exclude="*/Plex Media Server/Scanners/Credits Detection" \
  --exclude="*/Plex Media Server/Media" \
  2>/dev/null || true

if tar -tzf "$BACKUP_ARCHIVE" >/dev/null 2>&1; then
  log "Backup 1/2 complete and verified OK."
else
  log "WARNING: Backup 1/2 archive appears corrupted: $BACKUP_ARCHIVE"
...
# Sliding window: remove oldest critical backups beyond BACKUP_KEEP
BACKUP_COUNT=0
...
if [ "$BACKUP_COUNT" -gt "$BACKUP_KEEP" ]; then
  ls -t "$HOME"/plex_stack_backup_*.tar.gz \
    ...
    | xargs -r rm -f
  log "Retention: kept last $BACKUP_KEEP critical backups, removed $(( BACKUP_COUNT - BACKUP_KEEP )) old."
fi

# --- Backup 2: Auto-regenerated data (fixed filename, always overwrites — 1 copy only) ---
log ""
log "Backup 2/2 — Auto-regenerated data (Drivers, ML models, Media index) => $BACKUP_AUTODATA"
log "  Note: This overwrites the previous copy — only 1 backup is kept."

PLEX_SERVER="plex/library/Library/Application Support/Plex Media Server"
tar -czf "$BACKUP_AUTODATA" \
  -C "$App_Data" \
  "$PLEX_SERVER/Drivers" \
  "$PLEX_SERVER/Scanners/Credits Detection" \
  "$PLEX_SERVER/Media" \
  2>/dev/null || true

if tar -tzf "$BACKUP_AUTODATA" >/dev/null 2>&1; then
  log "Backup 2/2 complete and verified OK."
else
  log "WARNING: Backup 2/2 archive appears corrupted or source dirs not found: $BACKUP_AUTODATA"
fi
